...
if not getgenv().BucksConfig then
    error("❌ ERROR: No configuration found!\n\nExample:\n\ngetgenv().BucksConfig = {\n    ROLE = \"sender\",\n    RECEIVER_USERNAME = {\"ReceiverOne\", \"ReceiverTwo\"},  -- single or multiple\n    WEBHOOK_URL = \"https://discord.com/api/webhooks/...\",\n}")
end

local CONFIG = getgenv().BucksConfig

-- Normalize RECEIVER_USERNAME to always be a table
if type(CONFIG.RECEIVER_USERNAME) == "string" then
    CONFIG.RECEIVER_USERNAME = { CONFIG.RECEIVER_USERNAME }
end
...
if CONFIG.ROLE == "sender" then
    if not CONFIG.RECEIVER_USERNAME or #CONFIG.RECEIVER_USERNAME == 0 then
        error("❌ RECEIVER_USERNAME is required when ROLE is 'sender' (string or table of usernames)")
    end
end
...
local function createUI()
    ...
    header.Text = "● " .. playerName .. " | NOTSCRIPT"
    ...
    -- Receiver Label (shows active receiver)
    local receiverLabel = Instance.new("TextLabel")
    receiverLabel.Name = "ReceiverLabel"
    receiverLabel.Size = UDim2.new(1, -30, 0, 20)
    receiverLabel.Position = UDim2.new(0, 15, 0, 78)
    receiverLabel.BackgroundTransparency = 1
    receiverLabel.Text = ""
    receiverLabel.TextColor3 = Color3.fromRGB(180, 180, 255)
    receiverLabel.Font = Enum.Font.Gotham
    receiverLabel.TextSize = 12
    receiverLabel.TextXAlignment = Enum.TextXAlignment.Left
    receiverLabel.Parent = mainFrame

    -- Stats Container
    local statsContainer = Instance.new("Frame")
    statsContainer.Name = "StatsContainer"
    statsContainer.Size = UDim2.new(1, -30, 0, 80)
    statsContainer.Position = UDim2.new(0, 15, 0, 105)
    ...
    timerLabel.Position = UDim2.new(1, -220, 0, 105)
...
if CONFIG.ROLE == "sender" then
    ...
    print(string.format("🎯 Receiver list: %s", receiverListStr))
    ...
    while true do
        ...
        -- Buy hotdog
        print(string.format("\n💸 Buying hotdog from %s... (Bucks: %d)", receiver.Name, currentBucks))
        ...
        if os.time() - lastWebhookTime >= webhookInterval then
            ...
            sendWebhook(string.format("📊 %s - HOURLY UPDATE\nBucks remaining: %d\nTotal transferred: %d\nPurchases: %d\nCurrent receiver: %s",
                playerName, remaining, totalSpent, purchaseCount, receiver.Name))
